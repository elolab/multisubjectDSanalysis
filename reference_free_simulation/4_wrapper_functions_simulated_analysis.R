...
RunscTransform2wilcox <- function(whole.raw.data,whole.individual,whole.group,raw.data,normalized.data,individual,group)
{
    library(Seurat, quietly = TRUE)
    
    
    names(group) <- colnames(raw.data)
    names(individual) <- colnames(raw.data)
    group <- factor(plyr::mapvalues(x = group,from = names(table(group)),to = c("Control","Case")))
    
    names(whole.group) <- colnames(whole.raw.data)
    names(whole.individual) <- colnames(whole.raw.data)
    whole.group <- factor(plyr::mapvalues(x = whole.group,from = names(table(whole.group)),to = c("Control","Case")))
    
    so <- Seurat::CreateSeuratObject(whole.raw.data)
    
    so[["individual"]] <- whole.individual
    so[["group"]] <- whole.group
    
    so.list <- SplitObject(so, split.by = "individual")
    
    so.list <- lapply(so.list, function(x) SCTransform(x, vst.flavor = "v2", verbose = FALSE,return.only.var.genes=FALSE))
    
    so <- so.list[[1]]
    for (i in 2:length(so.list))
    {
        so <- merge(so,so.list[[i]])
    }
    
    
    ## This doesn't work! Raises an error. -> Share genes cannot be used.
    # shared_genes <- Reduce(intersect,lapply(so.list,function(x) rownames(x@assays$SCT)))
    # 
    # print(dim(so))
    # 
    # so <- so[shared_genes,]
    # 
    # print(dim(so))
    # print(head(colnames(so)))
    
    so <- PrepSCTFindMarkers(so)
    
    group_so <- so[["group"]]$group
    
    print(head(group_so))
    
    names(group_so) <- colnames(so)
    
    group_other <- paste0(group_so,"_0")
    names(group_other) <- colnames(so)
    group_other[names(group)] <- paste0(group_so[names(group)],"_1")
    
    so[["group_other"]] <- group_other
    
    Idents(so) <- "group_other"
    
    markers <- FindMarkers(so,assay = "SCT",ident.1="Case_1",ident.2="Control_1",logfc.threshold = 0,min.pct = 0,test.use="wilcox",verbose = FALSE)
